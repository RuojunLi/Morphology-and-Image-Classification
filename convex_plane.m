% function  sel_cell=convex_plane()
...
File_number=10;
File_pathB=['E:\plane_char\'];%char_fill_reduce to maxin_cell
File_pathC=['E:\plane_fill\'];%char_fill_reduce to maxin_cell
File_pathD='E:\data_0422\';%char_fill_reduce to maxin_density
% mkdir(File_pathD);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
File_number=10;
...
for Class=1:File_number
    ...
    File_name=[File_pathH,'set_plane'];%char_fill_reduce to maxin_cell
    load(eval('File_name'),'char_fill');%char_fill_reduce to maxin_cell
    File_name=[File_pathH,'set_norm'];%char_fill_reduce to maxin_cell
    ...
    data_count=100;
    rad_char(diff_char<data_count)=0;
    rad_fill(diff_fill<data_count)=0;
    [max_char,post_char]=max(rad_char);
    [max_fill,post_fill]=max(rad_fill);
    [max_value,max_class]=max([max_char,max_fill]);
    if max_class==1
        char_max=cell_char{post_char,1};
        fill_max=cell_char{post_char,2};
        cover_class{loop_p,1}=[0 max_value post_char radius_char(post_char,1) char_fill(post_char,1)];
    else
        char_max=cell_fill{post_fill,1};
        fill_max=cell_fill{post_fill,2};
        cover_class{loop_p,1}=[1 max_value post_fill radius_fill(post_fill,1) char_fill(set_fill(post_fill),2) set_fill(post_fill)];
    end
    for k=1:size_char
    ...
    set_convex{1,1}=char_max;
    set_convex{1,2}=fill_max;
